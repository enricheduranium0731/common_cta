...
class commonThread() :
    ...
    def iTime(self,symbol,period,k):
        global dict_symbols
        return dict_symbols[symbol][arr_PeriodS[self.refArrayInd(arr_PeriodI,period)]].iloc[::-1].iloc[k,0]
        
    def iOpen(self,symbol,period,k):
        global dict_symbols
        return dict_symbols[symbol][arr_PeriodS[self.refArrayInd(arr_PeriodI,period)]].iloc[::-1].iloc[k,1]

    def iHigh(self,symbol,period,k):
        global dict_symbols
        return dict_symbols[symbol][arr_PeriodS[self.refArrayInd(arr_PeriodI,period)]].iloc[::-1].iloc[k,2]
        
    def iLow(self,symbol,period,k):
        global dict_symbols
        return dict_symbols[symbol][arr_PeriodS[self.refArrayInd(arr_PeriodI,period)]].iloc[::-1].iloc[k,3]
        
    def iClose(self,symbol,period,k):
        global dict_symbols
        return dict_symbols[symbol][arr_PeriodS[self.refArrayInd(arr_PeriodI,period)]].iloc[::-1].iloc[k,4]

    def iVoiume(self,symbol,period,k):
        global dict_symbols
        return dict_symbols[symbol][arr_PeriodS[self.refArrayInd(arr_PeriodI,period)]].iloc[::-1].iloc[k,5]
    ...
    def getZoomHigh(self,symbol,period,time1,time2):
        frm=0
        while self.iTime(symbol,period,frm)>time1:
            frm += 1
 
        to=0
        while self.iTime(symbol,period,to)>time2:
            to += 1 
        
        price=self.iHigh(symbol,period,self.getHighInd(symbol,frm-to,to,period))    
        return price

    def getZoomLow(self,symbol,period,time1,time2):
        frm=0
        while self.iTime(symbol,period,frm)>time1:
            frm += 1
        ...
        price=self.iLow(symbol,period,self.getLowInd(symbol,frm-to,to,period))    
        return price
            ...
            if (self.iMA(symbol, period, period2, iInd, iMaMethod, PRICE_CLOSE) >
                ...
                if (self.iMA(symbol, period, period1, iInd, iMaMethod, PRICE_CLOSE) >
                    ...
                    if (self.getMaGradient(symbol, period, period1, iInd, iMaMethod) > 0):
                        if self.iClose(symbol, period, iInd)<((self.iMA(symbol, period, period2, iInd, iMaMethod, PRICE_CLOSE))+2*self.iAtr(symbol,period,14,iInd)):
                            rst = True
    ...
    def iBandsA(self,symbol,timeframe,period, deviation, bands_shift, applied_price, mode, shift):
        ...
        """
        price=dict_symbols[symbol][arr_PeriodS[self.refArrayInd(arr_PeriodI,timeframe)]].iloc[:,4]
        price_data = price
    ...
    def iBands(self, symbol, period, ma_period, deviations, bands_shift, applied_price, mode, shift):
        global dict_symbols
        """优化后的布林带访问方法"""
        df = dict_symbols[symbol][arr_PeriodS[self.refArrayInd(arr_PeriodI,period)]].iloc[::-1]
        if mode == MODE_UPPER:
    ...
    def get_expma(self, symbol, period, iInd1, number):
        ...
        close_prices = dict_symbols[symbol][arr_PeriodS[self.refArrayInd(arr_PeriodI,period)]].iloc[::-1].iloc[:,4]
    ...
    def calculate_macd(self, symbol,period,index, short_window=12, long_window=26, signal_window=9):
        #prices= dict_symbols[symbol][arr_PeriodS[self.refArrayInd(arr_PeriodI,period)]].iloc[::-1].iloc[:,4][::-1]
        prices=self.get_prices(symbol, period, index, short_window+long_window+signal_window)
        #index=len(dict_symbols[symbol][arr_PeriodS[self.refArrayInd(arr_PeriodI,period)]].iloc[::-1])-index-1
    ...
    def iMacdUpInd(self,symbol, period, iInd):
        ...
        if (self.iMacd(symbol,period,15,30,12,PRICE_CLOSE,MODE_MAIN,xInd)>self.iMacd(symbol,period,15,30,12,PRICE_CLOSE,MODE_SIGNAL,xInd)):    
            while (self.iMacd(symbol,period,15,30,12,PRICE_CLOSE,MODE_MAIN,xInd)>self.iMacd(symbol,period,15,30,12,PRICE_CLOSE,MODE_SIGNAL,xInd)):
                xInd=xInd+1
                
                if (xInd>=len(dict_symbols[symbol][arr_PeriodS[self.refArrayInd(arr_PeriodI,period)]].iloc[::-1])):
                    break 
    ...
    def get_prices(self,symbol, period, index, total_period):
        ...
        # 从嵌套字典中获取数据
        try:
            prices = dict_symbols[symbol][arr_PeriodS[period_index]][::-1]
    ...
    def iKdjUpInd(self,symbol, period, iInd):
        ...
        if self.iStochastic(symbol,period,14,3,"SMA","MAIN",xInd) > self.iStochastic(symbol,period,14,3,"SMA","SIGNAL",xInd):
            while True:
                main_stoch = self.iStochastic(symbol,period,14,3,"SMA","MAIN",xInd)
                signal_stoch = self.iStochastic(symbol,period,14,3,"SMA","SIGNAL",xInd)
            ...
            if (self.iBands(symbol,period,19,3,0,PRICE_CLOSE,MODE_UPPER,shift)>self.iBands(symbol,period,19,3,0,PRICE_CLOSE,MODE_UPPER,shift+1)):        
                if (self.iClose(symbol,period,shift)>self.iBands(symbol,period,19,3,0,PRICE_CLOSE,MODE_MAIN,shift)):
                    ...
                    while (self.iClose(symbol,period,x)>=self.iBands(symbol,period,19,3,0,PRICE_CLOSE,MODE_MAIN,x)):
                        ...
                        if (x>=len(dict_symbols[symbol][arr_PeriodS[self.refArrayInd(arr_PeriodI,period)]].iloc[::-1])):
                            break                        
                        rst=x
                    #print(shift,rst,sep=",")    
        return rst
    ...
    def get_ma_up_id(self,symbol, period, i_zoom, i_ref_chan_count, iInd, i_ma_method):
        ...
        if d_ma5 > d_ma10:
            for x in range(0,i_zoom):
                d_ma5 = self.iMA(symbol, period, 5, iInd + (x), i_ma_method, PRICE_CLOSE)
                d_ma10 = self.iMA(symbol, period, 10, iInd + (x), i_ma_method, PRICE_CLOSE)

                d_ma52 = self.iMA(symbol, period, 5, iInd + (x) + 1, i_ma_method, PRICE_CLOSE)
                d_ma102 = self.iMA(symbol, period, 10, iInd + (x) + 1, i_ma_method, PRICE_CLOSE)
    ...
    def isMacdUp(self,symbol, period, iInd,iMacdBrokenrefKDataLimit):
        ...
        if  (iPreMacdCount>=iPreFixedMacdCount) and (iMacd_ind>0) and (iMacd_ind-iInd<=iMacdBrokenrefKDataLimit):
            ...
            if (self.iRsiUpInd(symbol, period, iInd ,12,30,40)>0) and (self.iRsiUpInd(symbol, period, iInd ,12,30,40)<iInd+6):
                return True   

            #z=self.isBandGoUp(symbol,period,iInd)    
            if self.isBandGoUp(symbol,period,iInd):
                return True
    ...
    def isMacdDown(self,symbol, period, iInd,iMacdBrokenrefKDataLimit):
        ...
        if  (iPreMacdCount>=iPreFixedMacdCount) and (iMacd_ind>0) and (iMacd_ind-iInd<=iMacdBrokenrefKDataLimit):
            ...
            if (self.iRsiDownInd(symbol, period, iInd ,12,70,60)>0) and (self.iRsiDownInd(symbol, period, iInd ,12,70,60)<iInd+6):
                return True

            if self.isBandGoDown(symbol,period,iInd):
                return True
    ...
    def isUpLine(self,symbol,period,ind):
        return self.iClose(symbol,period,ind)>self.iOpen(symbol,period,ind)
        
    def isDownLine(self,symbol,period,ind):
        return self.iClose(symbol,period,ind)<self.iOpen(symbol,period,ind)
    ...
    def isSpecAction(self,symbol,period,refCount,ind): 
        rst =False
        x=0
        while x<refCount:
            if self.getAvgSpace(symbol,period,1,ind+x)>self.getAvgSpace(symbol,period,refCount,ind)*5:
                rst =True
            x+=1
        return rst

    def print_comment(self,symbol,dect_type,comment):
        global dict_comment
        isOk=False
        ...
        comments=dict_comment.get(symbol,None)
        if (not comments==None): 
            target_date=comments.get(comment,0) 
            if (not target_date==0):
                current_date = (datetime.now() - timedelta(minutes=1))            
                if current_date<target_date:
                    isOk=False
                else:                   
                    comments[comment]=datetime.now()
            else:
                comments[comment]=datetime.now()          
        ...
        dict_comment[symbol]=comments
        
        if isOk:
            print(comment)
            
    def isCanDeal(self,symbol,period,dect_type,type,strategy,ind,comment):
        ...
        if deal_eng.get_holding("normal")>=maxHoldinhg:
            return False
        ...
        if type=="BUY":                      
            ...
            while (((refId1-xInd)>=0) and (xInd<=2)):
                if (self.iClose(symbol,1440,refId1-xInd)<self.iLow(symbol,1440,refId1)):
                    isRevserseOk=True
                    xTmp=refId1
                xInd+=1
            ...
            while (((refId1-xInd)>=0) and (xInd<=2)):
                if (self.iClose(symbol,1440,refId1-xInd)>self.iHigh(symbol,1440,refId1)):
            ...
            for x in [y for y in arr_PeriodI if (y>=240 and y<=1440)]:
                xInd=period*ind//x
                if (self.iRsiDownInd(symbol, x, xInd ,12,70,60)>0) and (self.iRsiDownInd(symbol, x, xInd ,12,70,60)<xInd+6):
            ...
            for x in [y for y in arr_PeriodI if (y>=240 and y<=1440)]:
                ...
                xInd=period*ind//x          
                for method in arr_MaMethod:
                    if self.isPowerMaUp(symbol,30,60,100,x,method,xInd,16) or self.isPowerMaUp(symbol,21,89,144,x,method,xInd,16):
                        if (self.isTouchedBottomBand(symbol,x,xInd)):
                            cnt+=1      
                            
                        y=self.iMacdUpInd(symbol,x,xInd)
                        if ((y>0) and ((y-xInd)<6)):
                            cnt+=1
                        ...
                        if (self.isBottomChan(symbol,x,xInd,12)>=0):
                            cnt+=1
                        ...
                        for y in range(5,10):
                            if (self.iContinueUpFromBottomByLowId(symbol,iContinueCount2,xInd,x,y,iRefContinueCount)>0):
                                cnt+=1
            ...
            if deal_eng.isNNDealOk("BUY") and (run_type=="real"):
                isTrendOk=True
                            
            # if dect_type=="trend" and isTrendOk:
                # return isOk

            if self.isContinueDown(symbol,240,period*ind//240+1,3):
                self.print_comment(symbol,dect_type,"标的:("+symbol+"),周期:("+str(240)+")持续向下!!")
                isOk=False

            if self.isContinueDown(symbol,1440,period*ind//1440+1,3):
            ...
            if strategy!="superma":                                                
                for x in [y for y in arr_PeriodI if (y>=240 and y<1440)]:
                    xInd=period*ind//x
                    for y in range(5,10):
                        if (self.isContinueTrendDown(symbol,iContinueCount2,xInd,x,y,iRefContinueCount))  and (not isTrendOk):             
                            self.print_comment(symbol,dect_type,"标的:("+symbol+"),周期:("+str(period)+")分钟趋势持续向下!!")
                            isOk=False                                    

            if deal_eng.isNNDealOk("SELL") and (run_type=="real"):
            ...
            if deal_eng.isSNNDealOk(symbol,"BUY") and deal_eng.isNNDealOk("BUY") and (run_type=="real"):
                isOk=True
                
            for u in [y for y in arr_PeriodI if (y>=240 and y<=1440)]:
                xInd=period*ind//u
                y=self.iKdjDownInd(symbol,u,xInd)
                if ((y>=xInd) and ((y-xInd)<9)):
                    isOk=False
            ...
            for u in [t for t in arr_PeriodI if (t>=30 and t<=240)]:
                xInd=period*ind//u
                for y in range(5,10):                       
                    if (self.iContinueDownFromTopByHighId(symbol,iContinueCount2,xInd,u,y,iRefContinueCount)>0):
                        isOk=False 

            for u in [y for y in arr_PeriodI if (y>=30 and y<=240)]:
                xInd=period*ind//u                                    
            ...
            for x in [y for y in arr_PeriodI if (y>=240 and y<=1440)]:
                xInd=period*ind//x  
                if self.isPowerMaDown(symbol,21,89,144,x,arr_MaMethod[0],xInd,16):
                    isOk=False

                if self.isPowerMaDown(symbol,30,60,100,x,arr_MaMethod[0],xInd,16):
                    isOk=False

                if self.isPowerMaDown(symbol,10,30,60,x,arr_MaMethod[0],xInd,16):
                    isOk=False
            ...
            # xInd=period*ind//240
            # if self.iClose(symbol,period,ind)>self.iMA(symbol,240,8,xInd,arr_MaMethod[0],PRICE_CLOSE):
                # return False

            refId1=self.getRefHighInd(symbol, iContinueCount,period*ind//240,240,9)
            if (not isTrendOk):
                if (self.isPierceAndSwallowDownP(symbol,240,refId1,1.3)):            
                    refId2=self.getRefLowInd(symbol, iContinueCount,period*ind//240,240,9)    
                    if not ((refId2<refId1) and (self.isPierceAndSwallowUpP(symbol,240,refId2,1.3))):
                        self.print_comment(symbol,dect_type,"标的:("+symbol+"),周期:("+str(240)+")分钟向下吞没!!")
                        isOk=False                        

            refId1=self.getRefHighInd(symbol, iContinueCount,period*ind//1440,1440,9)
            refId2=self.getRefLowInd(symbol, iContinueCount,period*ind//1440,1440,9)
            if (not isTrendOk):
                if (refId1<refId2) and(self.isPierceAndSwallowDownP(symbol,1440,refId1,1.3)):
                    refId1=self.getRefHighInd(symbol, iContinueCount,period*ind//240,240,9)
                    refId2=self.getRefLowInd(symbol, iContinueCount,period*ind//240,240,9)    
                    if not ((refId2<refId1) and (self.isPierceAndSwallowUpP(symbol,240,refId2,1.3))):                
            ...
            if (self.getRefLowInd(symbol, iContinueCount,period*ind//1440,1440,12)<=period*ind//1440+1):
                isOk=False
                refId1=self.getRefHighInd(symbol, iContinueCount,period*ind//240,240,9)
                refId2=self.getRefLowInd(symbol, iContinueCount,period*ind//240,240,9)    
                if not ((refId2<refId1) and (self.isPierceAndSwallowUpP(symbol,240,refId2,1.3))):              
            ...
            if (self.getRefLowInd(symbol, iContinueCount,period*ind//240,240,9)<=period*ind//240+2):
                if (True):  #if (not (self.isContinueUp(symbol,240,period*ind//240+1,3))):
                    self.print_comment(symbol,dect_type,"标的:("+symbol+"),周期:("+str(240)+")分钟追到顶部!!")
                    isOk=False 
                    
            if (self.getRefHighInd(symbol, iContinueCount,period*ind//1440,1440,9)<=period*ind//1440+1):
                if (True):
                    self.print_comment(symbol,dect_type,"标的:("+symbol+"),周期:("+str(1440)+")分钟追到顶部!!")
                    isOk=False 
            ...
            xInd=period*ind//15
            if (not self.isPosReady(symbol,15,type,dect_type,xInd)) and (dect_type=="real" or dect_type=="test"):
        ...
        if type=="SELL":
            ...
            for x in [y for y in arr_PeriodI if (y>=240 and y<=1440)]:
                xInd=period*ind//x                
                if (self.iRsiUpInd(symbol, x, xInd ,12,30,40)>0) and (self.iRsiUpInd(symbol, x, xInd ,12,30,40)<xInd+6):
            ...
            for x in [y for y in arr_PeriodI if (y>=240 and y<=1440)]:
                ...
                xInd=period*ind//x          
                for method in arr_MaMethod:
                    if (self.isPowerMaDown(symbol,30,60,100,x,method,xInd,16)) or (self.isPowerMaDown(symbol,21,89,144,x,method,xInd,16)):
                        if (self.isTouchedTopBand(symbol,x,xInd)):
                            cnt+=1  
                            
                        y=self.iMacdDownInd(symbol,x,xInd)
                        if ((y>0) and ((y-xInd)<6)):
                            cnt+=1
                            #break
                        ...
                        if (self.isTopChan(symbol,x,xInd,12))>=0:
                            cnt+=1
                            #break
                        ...
                        for y in range(5,10):
                            if (self.iContinueDownFromTopByHighId(symbol,iContinueCount2,xInd,x,y,iRefContinueCount)>0):
                                cnt+=1
                                #break
            ...
            if self.isContinueUp(symbol,240,period*ind//240+1,3):
                self.print_comment(symbol,dect_type,"标的:("+symbol+"),周期:("+str(240)+")持续向上!!")
                isOk=False  

            if self.isContinueUp(symbol,1440,period*ind//1440+1,3):
            ...
            if strategy!="superma":    
                ...
                for x in [y for y in arr_PeriodI if (y>=240 and y<=1440)]:
                    xInd=period*ind//x
                    for y in range(5,10):    
                        if (self.isContinueTrendUp(symbol,iContinueCount2,xInd,x,y,iRefContinueCount)) and (not isTrendOk):            
                            self.print_comment(symbol,dect_type,"标的:("+symbol+"),周期:("+str(period)+")分钟趋势持续向上!!")
                            isOk=False                                              

            if deal_eng.isSNNDealOk(symbol,"SELL") and deal_eng.isNNDealOk("SELL") and (run_type=="real"):
                isOk=True

            for u in [y for y in arr_PeriodI if (y>=240 and y<=1440)]:
                xInd=period*ind//u
                y=self.iKdjUpInd(symbol,u,xInd)
                if ((y>=xInd) and ((y-xInd)<9)):
                    isOk=False
            ...
            for x in [y for y in arr_PeriodI if (y>=30 and y<=240)]:
                xInd=period*ind//x          
                if (self.isBottomChan(symbol,x,xInd,16)>=0):
                    self.print_comment(symbol,dect_type,"标的:("+symbol+"),周期:("+str(x)+")分钟底部背驰缠绕趋势向上!!")
                    isOk=False                         
                    
            for x in [y for y in arr_PeriodI if (y>=240 and y<=1440)]:
                xInd=period*ind//x  
                if self.isPowerMaUp(symbol,21,89,144,x,arr_MaMethod[0],xInd,20):
                    isOk=False

                if self.isPowerMaUp(symbol,30,60,100,x,arr_MaMethod[0],xInd,20):
                    isOk=False

                if self.isPowerMaUp(symbol,10,30,60,x,arr_MaMethod[0],xInd,20):
                    isOk=False
            ...
            refId1=self.getRefLowInd(symbol, iContinueCount,period*ind//240,240,9)
            if (not isTrendOk):
                if (self.isPierceAndSwallowUpP(symbol,240,refId1, 1.3)):
                    refId2=self.getRefHighInd(symbol, iContinueCount,period*ind//240,240,9)    
                    if not ((refId2<refId1) and (self.isPierceAndSwallowDownP(symbol,240,refId2,1.3))):            
                        self.print_comment(symbol,dect_type,"标的:("+symbol+"),周期:("+str(240)+")分钟向上刺透!!")
                        isOk=False  

            refId1=self.getRefLowInd(symbol,iContinueCount,period*ind//1440,1440,9)
            refId2=self.getRefHighInd(symbol,iContinueCount,period*ind//1440,1440,9)
            if (not isTrendOk):
                if (refId1<refId2) and(self.isPierceAndSwallowUpP(symbol,1440,refId1, 1.3)):
                    refId1=self.getRefLowInd(symbol, iContinueCount,period*ind//240,240,9)
                    refId2=self.getRefHighInd(symbol, iContinueCount,period*ind//240,240,9)    
                    if not ((refId2<refId1) and (self.isPierceAndSwallowDownP(symbol,240,refId2,1.3))):                
                        self.print_comment(symbol,dect_type,"标的:("+symbol+"),周期:("+str(240)+")分钟向上刺透!!")
            ...
            if (self.getRefHighInd(symbol, iContinueCount,period*ind//240,240,9)<=period*ind//240):
                isOk=False

            if (self.getRefLowInd(symbol, iContinueCount,period*ind//240,240,9)<=period*ind//240+2):
                if (True):  #if (not (self.isContinueDown(symbol,240,period*ind//240+1,3))):                
                    self.print_comment(symbol,dect_type,"标的:("+symbol+"),周期:("+str(240)+")分钟追到底部!!")
                    isOk=False 
                    
            if (self.getRefLowInd(symbol, iContinueCount,period*ind//1440,1440,9)<=period*ind//1440+1):
                if (True):
